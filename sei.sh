...
sleep 1 && curl -s https://raw.githubusercontent.com/applejuice28/node/main/logo.sh | bash && sleep 1


if [ ! $NODENAME ]; then
read -p "Name Node: " NODENAME
...
echo -e "\e[1m\e[32m4 Nodes Complete... \e[0m" && sleep 1


echo -e '\n\e[42mNode Status\e[0m\n' && sleep 1
if [[ `service seid status | grep active` =~ "running" ]]; then
  echo -e "transaction successful \e[32mworking\e[39m!"
else
